...
vim.keymap.set("n", "m", function()
  ...
  if not char:match "%u" then return vim.cmd("norm! m" .. char) end
  vim.cmd("mark " .. char)
...
vim.keymap.set("n", "'", function()
  local mark = vim.fn.getcharstr()
  local char = mark2char(mark)
  if not char:match "%u" then return vim.cmd("norm! '" .. char) end

  local mark_pos = vim.api.nvim_get_mark(char, {})
  if mark_pos[1] == 0 then return vim.notify("No mark at " .. mark, vim.log.levels.WARN, { title = "Marks" }) end

  vim.fn.feedkeys("'" .. mark2char(mark), "n")
end, { desc = "Go to mark" })

-- List Marks -----------------------------------------------------------------
vim.keymap.set("n", "<Leader>mm", function() list_marks() end, { desc = "List marks" })
...
vim.keymap.set("n", "<Leader>md", function()
  ...
  -- Validate that we have a valid mark to delete
  if not char:match "[A-I]" then
    vim.notify("Invalid mark: " .. mark .. " (use 1-9)", vim.log.levels.WARN, { title = "Marks" })
    return
  end
